...
This study aimed to investigate the factors that contribute to coral bleaching and present them in an interactive app that was aimed at high-school leavers to educate them on the complexity of predicting coral bleaching. We aimed to test different models that could potentially help predict bleaching, testing classifiers as well as linear models. We focused on rugosity, as it has yet to be extensively researched, to determine if it was a significant factor for coral bleaching. We found that with our current estimate of rugosity, it was not significant. Our final model used a cross validated KNN classifier that was able to predict bleaching with an accuracy of 66%. While we acknowledge this is not a strong model, we hope our Shiny app provides a solid background for understanding coral bleaching and what rugosity is, and our suggestions for future studies are taken on. 
...
```{r fig2, fig.width=12,fig.height=6,fig.cap="Figure 2. Violin-Boxplot of Model accuracies"}
# Fig.2: Violin plot
...
cf_data_all %>%
  ...
  data_color(columns = 'Scalability',
             ...
             colors = cf_colour_r, 
             autocolor_text = FALSE) %>% 
  tab_source_note(source_note = md("Figure 3. Table of Model Results comparing Rugosity"))
```

We also tuned our k-value for our KNN model by running our model with different k-values and seeing which had the best accuracy. As shown below in Fig.4, our best k-value was k=9 as per indicated by the orange line. 

```{r KNN Hyperparameter Tuning, fig.cap="Figure 4. Plot of Hyper-parameter testing for KNN, with orange line indicating best k-value"}
ggplot(knn_fit) + 
