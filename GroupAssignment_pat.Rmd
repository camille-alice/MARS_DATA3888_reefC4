...
set.seed(3888)
reef_ungroup = reef_final %>% ungroup()
X = reef_ungroup %>% select(-c("Average_bleaching","Date","class","bleached","num_rugosity","rugosity"))
...
rnd_mean = mean(cv_50acc_rnd)
rnd_sd = sd(cv_50acc_rnd)

#F1-score for random effect
rnd_cf = confusionMatrix(as.factor(fitres_rnd), y_rnd,
                mode = "everything",
                positive="1")
...
#reef without "bleached" and other useless attributes
reef_ungroup = reef_final %>% ungroup()
X = reef_ungroup %>% select(-c("Average_bleaching","bleached","Reef.ID","Date","class","rugosity"))
...
#reef without "bleached" and other useless attributes
reef_ungroup = reef_final %>% ungroup()
X = reef_ungroup %>% select(-c("Average_bleaching","bleached","Reef.ID","Date","class","num_rugosity"))
...
```{r}
save(cv_50acc5_bin, cv_50acc5_sel, cv_50acc_rnd, cv_acc50_NB, cv_acc50_knn, cv_acc50_rf, cv_acc50_svm, 
     bin_cf, bin_sel_cf, rnd_cf, nb_cf, knn_cf, rf_cf, svm_cf, 
     bin_mean, sel_mean, rnd_mean, nb_mean, knn_mean, rf_mean, svm_mean,
     bin_sd, sel_sd, rnd_sd, nb_sd, knn_sd, rf_sd, svm_sd,
